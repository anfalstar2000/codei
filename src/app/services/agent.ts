...
export const createSallaDeveloperAgent = (config: AgentConfig) => {
...
export const runWorkflow = async (
  ...
  return await withTrace('salla-developer-agent-v1', async () => {
    ...
    const agent = createSallaDeveloperAgent(config);

    const currentConversation: AgentInputItem[] = [
      ...conversationHistory,
      {
        role: 'user',
    ...
    const result = await runner.run(agent, currentConversation);
    ...
    return {
      ...
      trace: {
        toolCalls: result.newItems
          .filter((item) => item.type === 'tool_call')
          .map((item: any, index) => ({
            id: `tool-${index}`,
            name: item.tool?.name || 'unknown',
            input: JSON.stringify(item.tool?.input || {}),
            output: JSON.stringify(item.tool?.output || {}),
            duration: item.duration || 0,
          })),
        searchQueries: result.newItems
          .filter((item: any) => item.type === 'tool_call' && item.tool?.name === 'web_search')
          .map((item: any, index) => ({
            id: `search-${index}`,
            query: item.tool?.input?.query || '',
            results: item.tool?.output?.results?.length || 0,
            duration: item.duration || 0,
          })),
        executionLogs: result.newItems.map((item: any, index) => ({
          id: `log-${index}`,
          message: `${item.type}: ${item.role || 'system'}`,
          timestamp: new Date(),
        })),
        metrics: {
